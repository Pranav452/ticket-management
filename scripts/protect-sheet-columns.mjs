...
const ROLES = {
  ...
  "Ops lead":        { who: ["YOGESH"] },
  "Documentation":   { who: ["YOGESH", "SAGAR", "SUSHANT"] },
  "BL desk":         { who: ["YOGESH", "SAGAR", "SUSHANT", "JAYWANT"] },
  "Yard / CFS":      { who: ["YOGESH", "PRAMOD"] },
  "Everyone":        { who: null },   // no protection — open to all editors
};

/** Yogesh works across the whole sheet, so he is on every column group. */
const ALL_COLUMNS = ["YOGESH"];
...
async function main() {
  ...
  for (const id of targets) {
    ...
    if (mode === "warn") {
      // Column protections block sorting: a sort rewrites every column at once,
      // so a range owned by someone else refuses it. Advisory mode keeps the
      // ownership prompt but lets the team sort and filter normally.
      ...
      for (let i = 0; i < requests.length; i += 100) {
        await api(token, `https://sheets.googleapis.com/v4/spreadsheets/${id}:batchUpdate`, {
          method: "POST", body: JSON.stringify({ requests: requests.slice(i, i + 100) }),
        });
      }
      console.log(`  ${requests.length} protection(s) are now advisory (warn on edit, sorting works)`);
      continue;
    }
    ...
    // (--access-tab refreshes only this, leaving protections untouched.)
    const existingAccess = (meta.sheets ?? []).find((s) => normTab(s.properties.title) === normTab(ACCESS_TAB));
    const roleRow = (role) => [
      role,
      ROLES[role].who ? ROLES[role].who.join(", ") : "all editors",
      Object.entries(FIELD_POLICY).filter(([, r]) => r === role).map(([f]) => f).join(", ") || "—",
    ];
    const matrix = [
      ["COLUMN OWNERSHIP — who is responsible for what", "", ""],
      ["Applies to every country tab. Editing a column you do not own shows a confirmation prompt — a guardrail, not a lock, so sorting and filtering keep working for everyone.", "", ""],
      ["", "", ""],
      ["ROLE", "PEOPLE", "COLUMNS"],
      ...Object.keys(ROLES).map(roleRow),
      ["", "", ""],
      [`${ALL_COLUMNS.join(", ")} has access to every column — works across the whole sheet.`, "", ""],
      ["Admins", "Nakul Tanna, Pranav Nair", "everything, including the header row"],
      ["Read-only", "anyone else with the link", "no ownership on any column"],
      ["", "", ""],
      ["EMAILS", "", ""],
      ...Object.entries(P).map(([n, e]) => [n, e, ""]),
      ["", "", ""],
      ["Do not insert rows above the header row — it breaks the dashboard sync.", "", ""],
      ["For personal sorting use Data > Filter views; the shared filter changes the view for everyone.", "", ""],
      [`Generated ${new Date().toISOString().slice(0, 10)} by scripts/protect-sheet-columns.mjs`, "", ""],
    ];
    if (mode === "apply" || mode === "tab") {
      if (!existingAccess) {
        await api(token, `https://sheets.googleapis.com/v4/spreadsheets/${id}:batchUpdate`, {
          method: "POST",
